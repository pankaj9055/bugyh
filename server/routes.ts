...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/investments", authenticateToken, async (req: any, res) => {
    try {
      ...
      if (!user || parseFloat(user.balance) < parseFloat(plan.amount)) {
      ...
      // Add daily return to balance
      await storage.updateUserBalance(req.user.id, dailyReturnAmount);
      ...
      const { db, dailyReturns } = await import('./db');
  ...
  app.post("/api/deposit", authenticateToken, async (req: any, res) => {
    try {
      const { amount, transactionId, paymentMethod, paymentProof } = req.body;
      ...
      // Process referral commission when deposit is approved (will be called when admin approves)
      // Note: Commission is processed when admin approves the deposit
      ...
      const transaction = await storage.createTransaction({
        ...
        amount: amount.toString(),
        status: "pending",
        description: `${paymentMethod.toUpperCase()} deposit of ₹${parseFloat(amount).toLocaleString()}`,
  ...
  app.get("/api/admin/transactions", authenticateToken, requireAdmin, async (req: any, res) => {
    try {
      ...
      // For each transaction, get user details including UPI info for withdrawals
  ...
  app.post("/api/admin/payment-methods", authenticateToken, requireAdmin, async (req, res) => {
    try {
  ...
  app.put("/api/admin/payment-methods/:id", authenticateToken, requireAdmin, async (req, res) => {
    try {
      const { id } = req.params;
      const updates = req.body;
